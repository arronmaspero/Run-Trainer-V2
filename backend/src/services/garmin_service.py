...
def push_plan_to_garmin(user_id: str, db: Session, force_clear: bool = False) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        "2. MAIN SET & STEP GENERATION RULES:\n"
        "   - You MUST generate a separate Garmin watch step (step_type='interval') for EVERY item listed in main_set_instructions!\n"
        "   - For example, if main_set_instructions contains multiple steps (e.g., 'Run 7.5 miles easy' AND 'Increase pace for final 1.5 miles to 9:15-9:25 min/mi'), you MUST generate 2 distinct run steps: Step 1 (distance=7.5 miles, target pace=easy) AND Step 2 (distance=1.5 miles, target pace=9:15-9:25 min/mi).\n"
        "   - Do NOT collapse multiple main_set instructions into a single step!\n"
        "   - Generate warmup (step_type='warmup') and cooldown (step_type='cooldown') steps if warm_up_instructions and cool_down_instructions are non-empty.\n"
        ...
        "5. Calculate target values for speed (meters per second) if target_type is 'pace':\n"
        ...
        "6. Calculate the target speed in m/s for EACH individual step using that specific step's target pace if specified in text, or using the session's target_pace_range.\n"
        f"7. For Quality workout warmup and cooldown steps (step_type='warmup' or 'cooldown'): if easy jogging, set target_type to 'pace' using the Easy Pace Range: '{easy_pace_range}'. If walking or stretching, set target_type to 'no_target'.\n"
        "8. Ensure the estimatedDurationInSecs is the sum of all steps' durations.\n"
    )
